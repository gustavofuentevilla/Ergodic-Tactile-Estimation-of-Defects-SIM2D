...
% Remove initial found defect element 
...
Preprocessed_V = V;
Preprocessed_V(idx_V) = V(idx_V)*DataEscFact; %Scaling data above threshold
Preprocessed_V(~idx_V) = V(~idx_V)*0; %throwing away data under threshold

Data_current = [X_e, Preprocessed_V];

% This approach doesn't use historical data for estimation
Data = Data_current;
...
if ~isempty(Prev_Mu_found)        %If some defect has been found
    ...
    % Remove current data points that are inside the already found defects
    idx_remData = IsDataInEllipse(Data, Prev_Mu_found, Prev_Sigma_found);
    Data(idx_remData,:) = [];

    % Remove Pix related to data points that WERE inside found
    % defects ellipses in previous iterations
    idx_remData = IsDataInEllipse(Prev_Data_Xe_hist_V,...
                                  Prev_Mu_found, Prev_Sigma_found);
    Prev_Pix(idx_remData,:) = [];
...
[Priors, Mu, Sigma, Pix] = GMM_inc_EM(Data_Xe_hist_V, K, ...
                                      Prev_Priors, Prev_Mu, Prev_Sigma,...
                                      Prev_Pix, iteration,...
...
%% Minimum variance and Minimum axis lengths constraints: re-adjustment of Sigma
