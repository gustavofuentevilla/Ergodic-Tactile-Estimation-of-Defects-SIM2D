...
%Testing 
% Mu = [1.3938    0.7218;	    0.7558    0.8584;	    1.2323    1.2161];
% Mu = [1.3924    0.7547;    0.6860    1.4378;    1.4159    1.3244];
...
load(sprintf("/home/gustavo-fuentevilla/MATLAB/Tactile_Defects_Localization/Exploration_of_Defects/2. GMM_approach/4.1. Tactile_Exp_GMM_Appr_ndef_known/Results100/output_%d.mat",run_idx), "Mu", "Sigma")
...
Par_PDF.x_2 = x_2;
Par_PDF.Meas_mean = a;
Par_PDF.K = n_def; % K Gaussian Elements
Par_PDF.Omega = Omega;

Par_PDF.DataEscFact = 1;% 5;
Par_PDF.Thres_Variation = 0.135; % 13 cm total variation condition to find a defect
Par_PDF.MinAxisLengths = 0;% 0.04; % 4 cm Minimum axes lengths of gaussian elipses

% Par_PDF.Prev_Data = [];
Par_PDF.Prev_Data_Xe_hist_V = [];
Par_PDF.Prev_Pix = [];
Par_PDF.Prev_Priors = [];
...
for i = 1:n_iter_max
    ...
    % MinAxisLength definition for a minimum covariance constraint
    minVariation_up = 1.5;    % 1.5 m
    minVariation_low = Par_PDF.Thres_Variation - 0.001; % should be < than Thres_Variation
    minIter = 4;
    if i <= minIter
        Par_PDF.MinVariation = (i - 1)*(minVariation_low - minVariation_up)/(minIter - 1) +...
                                minVariation_up;
    else
        Par_PDF.MinVariation = minVariation_low;
    end
    ...
    Par_PDF.Prev_Data_Xe_hist_V = Estim_sol(i).Data_Xe_hist_V;
    Par_PDF.Prev_Pix = Estim_sol(i).Pix;
    ...
    % Update number of gaussians for next estimation
    Par_PDF.K = n_def - (size(Par_PDF.Prev_Mu_found, 1) - 1);
...
% Normal for-loop
save(sprintf("Results100/output_%d.mat",run_idx), "-regexp", "^(?!(M)$).");
